...
$.fn.fixed_share = function () {

    $(this).click(on_btn_click);
    function on_btn_click(e) {
        ...
        var $btn = $(this),
            ...
            share_url = $btn.parent().parent().data('share-url'),
            text = $btn.data('text') || $btn.parent().parent().data('text'),
            url_encoded = encodeURIComponent(share_url),
            url = '',
            cls = $btn.attr('class'),
        ...
        if (cls.indexOf('twitter') !== -1) {
            url += 'http://twitter.com/share?url=' + url_encoded +
            '&text=' + encodeURIComponent(text);
        } else if (cls.indexOf('facebook') !== -1) {
            url += 'http://www.facebook.com/share.php?u=' + url_encoded;
        } else if (cls.indexOf('google') !== -1) {
            url += 'https://plus.google.com/share?url=' + url_encoded;
        } else if (cls.indexOf('pinterest') !== -1) {
            url += 'http://pinterest.com/pin/create/button/?url=' + url_encoded +
            '&media=' + encodeURIComponent($('meta[property="og:image"]').attr('content')) +
            '&description=' + encodeURIComponent(
                item_name + '.\n' + $('meta[name=description]').attr("content")
            );
        } else if (cls.indexOf('tumblr') !== -1) {
            url += 'http://www.tumblr.com/share/link?url=' + url_encoded +
            '&name=' + encodeURIComponent(item_name) +
            '&description=' + encodeURIComponent($('meta[name=description]').attr("content"));

        } else if (cls.indexOf('mail') !== -1) {
            url += '?subject=ListFellas.com - ' + encodeURIComponent(item_name) +
            '&body=' + encodeURIComponent(
                'ListFellas.com - ' + item_name + '\n' + share_url + '\n\n' +
                $('meta[name=description]').attr("content")
            );
        ...
        var win = window.open(url, 'listfellas-share', opts);
        win.focus();
...
CssMatrix = (function () {
    ...
    var vendor_prefixes = ['transform', '-webkit-transform', '-ms-transform'];
    function apply($elem, from, to) {
        var css = get_css(from, to);
        for(var a in vendor_prefixes){
            $elem.css(vendor_prefixes[a], css);
        }
        return $elem;
    }
...
// In case FireBug is not installed
...
MM.Scroll = function () {
    ...
    function init() {
        $('#sidebar-scrollable').perfectScrollbar({
...
MM.OffCanvas = function () {
    ...
    function init() {
        $toggle.on('click', showMenu);
    }

    function showMenu(e) {
        $body = $body || $('body');
        $body.addClass(active_class);
        $toggle.off('click', showMenu);
        e.stopPropagation();
        $(document).on('click', onDocumentClick);
    }

    function onDocumentClick(e) {
        if ($(e.target).closest('.sidebar-right').length) {
            return;
        }
        hideMenu();
    }
...
MM.MockupMatrix = function () {
    var $areas, $bg_image, prev_iw, active_mockup;

    function onAreasLoaded($mockup_areas, _active_mockup) {
        prev_iw = null;
        $areas = $mockup_areas;
        $(window).off('resize', transformMockup);
        $(window).on('resize', transformMockup);
        transformMockup(null, _active_mockup);
    }

    function bgLoaded(_active_mockup) {
        $bg_image = $('#bg_image');
        prev_iw = null;
        transformMockup(null, _active_mockup);
    }

    function transformMockup(e, _active_mockup) {
        var iw = ($bg_image ? $bg_image.width() : null), scale_ratio, bg_width;
        active_mockup = _active_mockup || active_mockup;

        if (!$areas.length || !$bg_image || !iw || (iw >= 960 && !prev_iw) || iw == prev_iw) {
            return;
        }

        scale_ratio = parseInt(active_mockup.bg.w) / iw;
        prev_iw = iw;
        ...
        $areas.each(function () {
            ...
            for (var a in active_mockup.areas) {
                if (active_mockup.areas[a].id == area_id) {
                    area_data = active_mockup.areas[a];
                    break;
                }
            }

            var area_ratio = area_data.width / area_data.height;
            if ($area.hasClass('area-screen')) {
                pw = bg_width / area_data.scale;
                ph = pw / area_ratio;
                pw = Math.round(pw);
                ph = Math.round(ph);
            } else {
                pw = area_data.width / 3;
                ph = area_data.height / 3;
                pw = Math.round(pw);
                ph = Math.round(ph);
                $area.find('.area-text').width(pw).height(ph);
            }
            ...
            var from = [
                ...
                to = scale_points(area_data.points, scale_ratio);

            //console.info('css transform');
            CssMatrix.apply($area, from, to);
        });
    }

    function scale_points(points, scale_ratio) {
        var new_points = [];
        for (var a in points) {
            new_points.push({
                x: Math.round(points[a].x / scale_ratio),
                y: Math.round(points[a].y / scale_ratio)
            });
        }
        return new_points;
    }
    ...
    return {
        onAreasLoaded: onAreasLoaded,
        bgLoaded: bgLoaded
    }
}();

MM.setWindowTitle = function (mockup) {
    var title = 'Magic Mockups &bull; Free real-life mockup generator';
    if (mockup) {
        title = mockup.name + ' - ' + title;
    }
    MM.$winTitle && MM.$winTitle.html(title);
};

$.fn.mockupToggleFilters = function () {
    this.each(function () {
        var target_cls = $(this).data('target');
        $(this).find('a').each(function () {
            $(this).on('click', function (e) {
                //e.preventDefault();
                onFilterClick($(this), target_cls)
            });
        })
    });

    function onFilterClick($link, target_cls) {
        $link.parent().parent().find('.active').removeClass('active');
        $link.parent().addClass('active');
        var filterValue = $link.data('filter');
        if (filterValue) {
            $('.' + target_cls).addClass('hide');
            $('.' + target_cls + '-' + filterValue).removeClass('hide');
        } else {
            $('.' + target_cls).removeClass('hide');
        }
        $('#sidebar-scrollable').perfectScrollbar('update');
    }
};
...
$(function () {
    ...
    $('.filters').mockupToggleFilters();
    $('.share-horizontal a').fixed_share();
});
